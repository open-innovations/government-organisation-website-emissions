		...
		tables.forEach(function(t){
			...
			if(tr.length > 3){
				...
				for(d = sdate; d <= edate; d.setDate(d.getDate() + 1)){
					...
					m = d.getMonth();
					if(!xlabels[smonth]) xlabels[smonth] = {'label':monthNames[m]+(m==0 ? ' '+d.getFullYear():'')};
				}
